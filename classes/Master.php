...
Class Master extends DBConnection {
	...
	function save_bo(){
		...
		if (empty($id)){
			$sql="INSERT INTO `back_order_list` set {$data}";
		}else{
			$sql = "UPDATE `back_order_list` set {$data} where id='{$po_id}'";
		}
		$save = $this->conn->query($sql);
		if($save){
			$resp['status']='success';
			...
			foreach($item_id as $k =>$v){
				if(!empty($data)) $data .=", ";
				$data .= "('{$bo_id}','{$v}','{$qty[$k]}','{$price[$k]}','{$unit[$k]}','{$total[$k]}')";
			}
			if(!empty($data)){
				$this->conn->query("DELETE FROM `bo_items` where bo_id='{$bo_id}'");
				$save = $this->conn->query("INSERT INTO `bo_items` (`bo_id`, `item_id`, `quantity`, `price`, `unit`, `total`) VALUES {$data}");
				if(!$save){
					$resp['status']='failed';
					if(empty($id)){
						$this->conn->query("DELETE FROM `back_order_list` where id='{$bo_id}'");
					}
					$resp['msg']='BO has failed to save. Error: '.$this->conn->error;
					$resp['sql']="INSERT INTO `bo_items` (`bo_id`, `item_id`, `quantity`, `price`, `unit`, `total`) VALUES {$data}";
				}
			}
		}else{
			$resp['status']='failed';
		...
		if($resp['status']=='success'){
			if(empty($id)){
				$this->settings->set_flashdata('success', "New Back Order was Successfully created.");
			}else{
				$this->settings->set_flashdata('success', "Back Order's Details Successfully updated.");
			}
		}
